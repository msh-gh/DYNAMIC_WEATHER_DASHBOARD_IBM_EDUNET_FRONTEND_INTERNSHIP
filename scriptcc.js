...
const url = `https://data.api.xweather.com/places/${city}?format=json&limit=10&client_id=${client_id}&client_secret=${client_secret}`
  async function placename() {
      ...
      if (!success || !response || !response.place) {
        console.error("placename API call failed or returned invalid data", data);
        document.getElementById('cityname').innerHTML = `<div class="city">Could not find location: ${newcity}</div>`;
...
// Get Weather
   ...
   async function getWx() {
      ...
      if (!success || !response || response.length === 0) {
        console.error("getWx API call failed or returned no data", data);
        return;
      }
...
tempC = response[0].periods[0].tempC
      ...
      document.getElementById('icon').innerHTML = `<img src="https://raw.githubusercontent.com/JesseWx2011/weather-dashboard/main/icons/${response[0].periods[0].icon}" style="width: 10%; padding-left: 590px; display: flex;" alt="Current weather icon">`;
    ...
    radar = document.getElementById("radarimage").innerHTML = `<img style="width: 617px;" src="https://maps.aerisapi.com/${client_id}_${client_secret}/flat-dk,water-depth,satellite,radar,alerts-warnings-outlines,counties,lightning-strikes-15m-icons,roads,interstates,rivers,admin-cities-dk/1280x878/${city},9/current.png" alt="Weather radar map"/>`      
   ...
   // Placeholder for if no matches for the if statements. 
   riskphrase1 = ""
   var risktype1 = response[0].details.risk.name
      // Use "|" instead of ","
   ...
   async function getForecast() {
      ...
      const { response, success } = data;
      if (!success || !response || response.length === 0) {
        console.error("getForecast API call failed or returned no data", data);
        return;
      }
      ...
      document.getElementById('wxicon0').innerHTML = `<img src="https://raw.githubusercontent.com/JesseWx2011/weather-dashboard/main/icons/${response[0].periods[0].icon}" alt="weather icon">`;
      ...
      document.getElementById('wxicon1').innerHTML = `<img src="https://raw.githubusercontent.com/JesseWx2011/weather-dashboard/main/icons/${response[0].periods[1].icon}" alt="weather icon">`;
      ...
      document.getElementById('wxicon2').innerHTML = `<img src="https://raw.githubusercontent.com/JesseWx2011/weather-dashboard/main/icons/${response[0].periods[2].icon}" alt="weather icon">`;
      ...
      document.getElementById('wxicon3').innerHTML = `<img src="https://raw.githubusercontent.com/JesseWx2011/weather-dashboard/main/icons/${response[0].periods[3].icon}" alt="weather icon">`;
      ...
      document.getElementById('wxicon4').innerHTML = `<img src="https://raw.githubusercontent.com/JesseWx2011/weather-dashboard/main/icons/${response[0].periods[4].icon}" alt="weather icon">`;
      ...
      document.getElementById('wxicon5').innerHTML = `<img src="https://raw.githubusercontent.com/JesseWx2011/weather-dashboard/main/icons/${response[0].periods[5].icon}" alt="weather icon">`;
      ...
      document.getElementById('wxicon6').innerHTML = `<img src="https://raw.githubusercontent.com/JesseWx2011/weather-dashboard/main/icons/${response[0].periods[6].icon}" alt="weather icon">`;
   ...
   async function getAlert() {
      ...
      if (!success || !response || response.length === 0) {
         alertsDiv.innerHTML = "No Alerts, Enjoy your day :)";
         return;
      }
      ...
      if (details?.name) {
         alertsDiv.innerHTML = `<div style="background-color: #${details.color};">Alert: ${details.name} in effect for ${place.name} ${place.state}.<a href="alertdetail.html?city=${city}">Click Here for more information on alerts</a></div>`
      } else {
         alertsDiv.innerHTML = "No Alerts, Enjoy your day :)";
      }
   }
